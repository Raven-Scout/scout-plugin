...
2. If the user named a tag or topic (e.g. `/scout-reply S2DA6B` or `/scout-reply ACME`), pick that
   draft. Otherwise show a compact list of `status: draft` items:
   ```
   Owed replies with a prepared draft:
   1. [#S2DA6B] Priya Rivera (ACME) — Re: Zmeny rolí  · email
   2. [#S39499] Jan Novák (Firma) — Jira Cloud API    · email
   …
