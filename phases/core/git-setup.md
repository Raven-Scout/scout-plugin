...
**Sanity-check cited times at compose time.** When citing a timestamp described as "this morning" / "earlier today," cross-check it against `TZ={{TIMEZONE}} date '+%H:%M'` — if the cited hour doesn't fit the narrative (an event described as pre-8 AM carrying an afternoon time), that's a UTC-copied-as-local smell; re-derive from the source string. **One event, one time:** a single event must never appear with two different times across surfaces in the same run (e.g. the raw UTC value in one line and the converted local time in another) — pick the display zone, convert once, and use it everywhere.

### Hard Rule — Weekday Derivation

Every DM, briefing, and file header that includes a day name (e.g., "Sat Apr 18", "Monday, April 20") **must** be generated by a fresh `TZ={{TIMEZONE}} date '+%A %b %-d'` (or `+%a %b %-d` for abbreviated) invocation *at compose time*. Never hand-write the weekday. Never reuse a weekday string from an earlier step or file — a string written once and copied across files drifts silently, and bare `date` in a UTC-clocked sandbox is off by a day near the timezone/UTC boundary.

```bash
TZ={{TIMEZONE}} date '+%A %b %-d'     # e.g., "Saturday Apr 18"
TZ={{TIMEZONE}} date '+%Y-%m-%d'      # date for filenames
```

**Pre-send assertion:** before sending any notification or writing any file header that contains a day name, verify the day name matches `TZ={{TIMEZONE}} date '+%A'` for the current moment. On mismatch, regenerate — do not send.

**Canonical filename pattern:** `action-items-YYYY-MM-DD.md`, with the date part from `TZ={{TIMEZONE}} date '+%Y-%m-%d'`. Any day-name written inside the file header must be derived from that same date via a fresh `date` call, not hand-written.
